...
// TODO: Replace with lerp from ./core?
...
export class Noise {
  ...
  private readonly grad: Vector3[] = new Array(512);
  ...
  public seed(seed: number) {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      this.grad[i] = this.grad[i + 256] = Grad3[v % 12];
    }
  }
  ...
  public perlin2(x: number, y: number): number {
    ...
    const n00 = this.grad[X + this.perm[Y]].dot(x, y);
    const n01 = this.grad[X + this.perm[Y + 1]].dot(x, y - 1);
    const n10 = this.grad[X + 1 + this.perm[Y]].dot(x - 1, y);
    const n11 = this.grad[X + 1 + this.perm[Y + 1]].dot(x - 1, y - 1);
